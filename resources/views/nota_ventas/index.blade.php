...
<div class="container mx-auto px-4 py-8" x-data="{ showDeleteConfirmModal: false, deleteFormAction: '' }">
    <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-3xl font-bold text-gray-800">Notas de Venta</h2>
            <a href="{{ route('nota_ventas.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-xl shadow-md">Nueva Venta</a>
        </div>
        ...
        <table class="min-w-full bg-white rounded-xl shadow-sm">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-2 text-xs font-medium text-gray-500 uppercase">Cliente</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($nota_ventas as $nota)
                <tr>
                    <td class="px-4 py-2">{{ $nota->id }}</td>
                    <td class="px-4 py-2">{{ $nota->fecha }}</td>
                    <td class="px-4 py-2">{{ $nota->monto }}</td>
                    <td class="px-4 py-2">{{ $nota->cliente->nombre }}</td>
                    <td class="px-4 py-2">{{ $nota->tipoPago->nombre }}</td>
                    <td class="px-4 py-2 space-x-2">
                        <a href="{{ route('nota_ventas.show', $nota) }}" class="text-blue-600 hover:text-blue-900">Ver</a>
                        <button @click="showDeleteConfirmModal = true; deleteFormAction = '{{ route('nota_ventas.destroy', $nota) }}'" class="text-red-600 hover:text-red-900">Eliminar</button>
                    </td>
                </tr>
                @empty
                <tr>
    ...
    <div x-show="showDeleteConfirmModal" x-transition class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full text-center" @click.away="showDeleteConfirmModal = false">
            <h3 class="text-xl font-bold mb-2 text-gray-800">¿Eliminar Venta?</h3>

...
@section('content')
<div class="container mx-auto px-4 py-8" x-data="notaEntrada()">
    <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8 max-w-4xl mx-auto">
        <h2 class="text-2xl font-bold text-gray-800 mb-4">Nueva Nota de Entrada</h2>

        <form method="POST" action="{{ route('nota_entradas.store') }}">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-gray-700 font-semibold mb-1">Fecha</label>
                    <input type="datetime-local" name="fecha" class="w-full border rounded-xl p-2" required>
                </div>
                <div>
            ...
            <div>
                ...
                <textarea name="descripcion" class="w-full border rounded-xl p-2"></textarea>
            </div>
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="index">
                            <tr>
                                ...
                                <td class="p-2">
                                    <input type="number" min="1" class="w-full border rounded-xl p-1" :name="`productos[${index}][cantidad]`" x-model.number="item.cantidad" @input="updateSubtotal(index)">
                                </td>
                                <td class="p-2">
                                    <input type="number" min="0" step="0.01" class="w-full border rounded-xl p-1" :name="`productos[${index}][precio_unitario]`" x-model.number="item.precio_unitario" @input="updateSubtotal(index)">
                                </td>
            ...
            <div class="mt-6 flex justify-end space-x-2">
                <a href="{{ route('nota_entradas.index') }}" class="px-4 py-2 bg-gray-300 rounded-xl">Cancelar</a>
